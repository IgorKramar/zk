...
**Primary:** Terminal-native инженеры/исследователи — они уже живут в vim+tmux+git и нанимают zk, чтобы вести Zettelkasten не выходя из своей цепочки инструментов.
...
- **Capture latency** — медиана времени от запуска `zk new` до сохранения связанной заметки; цель <5 сек. *Leading.*
- **Cross-session usage frequency** — заметки, созданные/отредактированные внутри coding-сессии (тот же tmux/git-repo как код). Регрессирует, если zk вытесняется обратно в Obsidian. *Leading.*
...
_Why it serves the approach:_ прямо воплощает research-grounded constraints — продукт отказывается от анти-паттернов. Без этого трека zk становится «ещё одним markdown-редактором с TUI».
