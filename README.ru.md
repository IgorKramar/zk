...
У проекта то же имя, что у [zk-org/zk](https://github.com/zk-org/zk) — действующего Go-CLI для Zettelkasten (~2,6k★, активный в мае 2026). Это разные проекты с разными целями:

- **zk-org/zk** методологически нейтрален: сироты-заметки, глубокие папочные иерархии и свободные теги — всё разрешено.
- **zk (этот проект)** принуждает к доказанным паттернам Zettelkasten (atomic notes, link-before-save, фиксированная ID-схема, отказ от папок-как-таксономии) на этапе записи, а не post factum.

Имя будет пересмотрено в ближайшем ADR (см. решение **D3** в [`decision-map.md`](./docs/architecture/decision-map.md)). До того момента `zk` — рабочее имя.
...
1. [`STRATEGY.md`](./STRATEGY.md) — что такое zk, кому он, ключевые метрики, треки работы.
